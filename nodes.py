...
class EasyNodeLoader: 
    ...
    def load_image(self, mode, image, batch_path, width, height, upscale_method, keep_proportion, crop_position, divisible_by, mask_data=""):
        ...
        if mode == "single" and mask_data:
            ...
            # 2. Fallback: Force check in input directory (fixes issues where annotated path fails for root files)
            ...
            if mask_path and os.path.exists(mask_path):
                try:
                    ...
                    print(f"[EasyNode] Error loading mask file {mask_path}: {e}")
            else:
                print(f"[EasyNode] Mask file not found: {mask_data}")
        ...
        for img_type, img_info in images_to_load:
            ...
            for i in ImageSequence.Iterator(img):
                ...
                # 1. Prepare Mask Logic
                if external_mask is not None:
                    current_mask_pil = Image.fromarray((external_mask * 255).astype(np.uint8), mode='L')
                    # Force mask to match image size FIRST before any other processing (SGUN Logic)
